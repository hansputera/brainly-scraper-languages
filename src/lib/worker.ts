...
import {
  CountryList, LanguageList, Brainly,
  JsonRes, Answer, Question,
} from '..';
 ...
 */
export async function search({c, language, question, length = 10, options}:{
    c: CountryList;
  ...
  try {
    ...
    const response = await Brainly.client(c).post(`graphql/${language.toLowerCase()}`, body, options);
    ...
    const objects = validJSON.map((obj) => {
      ...
      const answers: Answer[] = obj.node.answers.nodes.map(
          (answerObj) => Util.convertAnswer(answerObj));
      return {
        question, answers,
      };
    });
